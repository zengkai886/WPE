...
int main(int argc,char** argv){try{
    ...
    Check(DataService::NeedsOpenFile("mapCommand",{{"remote",false},{"action",8}})&&DataService::NeedsSaveFile("mapCommand",{{"remote",true},{"action",5}})&&DataService::FileKind("mapCommand",{{"remote",true}})=="pmr","Mapping chooser routing missing");
    Check(call("saveMapLocal",{{"host","local.example"},{"port",8080},{"remotePath","/assets"},{"localPath","C:\\映射\\index.html"}})["error"]=="","Standalone local map save failed");
    Check(call("saveMapRemote",{{"hostFrom","from.example"},{"portFrom",80},{"pathFrom","/old"},{"hostTo","to.example"},{"portTo",443},{"pathTo","/new"}})["error"]=="","Standalone remote map save failed");
    const auto pml=dir/"mapping.pml",pmr=dir/"mapping.pmr";call("mapCommand",{{"remote",false},{"action",5},{"_filePath",Path(pml)}});call("mapCommand",{{"remote",true},{"action",5},{"_filePath",Path(pmr)}});
    auto localXml=ParseXml(read(pml)),remoteXml=ParseXml(read(pmr));Check(localXml.LocalName()=="MapLocal"&&localXml.nodes.size()==1&&localXml.nodes[0].Value("ProtocolType")=="Http"&&localXml.nodes[0].Value("LocalPath")=="C:\\映射\\index.html","Local mapping XML shape differs from original");Check(remoteXml.LocalName()=="MapRemote"&&remoteXml.nodes.size()==1&&remoteXml.nodes[0].Value("Host_To")=="to.example"&&remoteXml.nodes[0].Value("Port_To")=="443","Remote mapping XML shape differs from original");
    const auto encryptedMap=dir/"mapping-encrypted.pml";call("mapCommand",{{"remote",false},{"action",5},{"_filePath",Path(encryptedMap)},{"_password","密码中文测试"}});Check(CryptXml(read(encryptedMap),"密码中文测试",false)==read(pml),"Encrypted mapping export did not round-trip");
    call("mapCommand",{{"remote",false},{"action",7}});call("mapCommand",{{"remote",false},{"action",8},{"_filePath",Path(pml)}});call("saveMapSetting",{{"enableLocal",true},{"enableRemote",true}});
    Check(call("saveServer",{{"enable",true},{"name","配置节点"},{"ip","127.0.0.1"},{"port",1080},{"forgotUrl","/forgot"},{"registerUrl","/register"},{"verifyUrl","/verify"}})["error"]=="","Backup server save failed");
    auto lastFeed=[&](int list){auto it=std::find_if(events.rbegin(),events.rend(),[&](const Json& event){return event["name"]=="feed:replace"&&event["data"]["list"]==list;});if(it==events.rend())throw std::runtime_error("Missing feed");return it->at("data").at("rows");};
    const auto serverId=lastFeed(17)[0]["Id"];Check(call("saveServerRule",{{"sid",serverId},{"enable",true},{"type",1},{"argument","example.com;example.org"},{"ruleAction",0}})["error"]=="","Backup server rule save failed");Check(call("saveNotice",{{"type",2},{"title","公告"},{"content","正文"},{"more","/more"}})["error"]=="","Backup notice save failed");
    const auto allExtra=dir/"extra-groups.sb";call("exportBackup",{{"proxyMapping",true},{"wpcServer",true},{"wpcNotice",true},{"_filePath",Path(allExtra)}});const auto extraRoot=ParseXml(read(allExtra));Check(extraRoot.Get("MapLocal")&&extraRoot.Get("MapRemote")&&extraRoot.Get("ServerList")&&extraRoot.Get("NoticeList"),"Completed backup groups missing");
    call("mapCommand",{{"remote",false},{"action",7}});call("mapCommand",{{"remote",true},{"action",7}});call("clearServers");call("clearNotices");call("importBackup",{{"_filePath",Path(allExtra)}});Check(lastFeed(13).size()==1&&lastFeed(14).size()==1&&lastFeed(17).size()==1&&lastFeed(17)[0]["RuleCount"]==2&&lastFeed(18).size()==1,"Three backup groups did not replace and restore data");
    ...
    auto withMaps=parts;withMaps["proxyMapping"]=true;withMaps["_filePath"]=Path(out);call("exportBackup",withMaps);Check(ParseXml(read(out)).Get("MapLocal")&&ParseXml(read(out)).Get("MapRemote"),"Supported mapping backup was omitted");
