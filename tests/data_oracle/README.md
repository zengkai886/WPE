...
`extract_contracts.py <包含Operate.cs的原项目目录> <C++项目目录>` 从原源码重提取：217 个 RPC、20 张建表 SQL、当前使用的 14 张表头文件，以及业务翻译/国家字典。此工具会覆盖生成文件，只供明确更新契约时使用；普通构建不运行它。
...
`verify-accounts-db <原生测试数据库> <输出.pa>` 让未修改原程序集加载 ProxyAccount/ProxyAccountIPInfo 并按原保存器重写 `.pa`；固定账号密码、登录记录与本地 C++ 导出逐字节一致。
