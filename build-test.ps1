...
$manifest=[ordered]@{
    ...
    scope='Protocol/ring, real three-channel Windows named-pipe transport with current-user ACL, shell/target session lifecycle, target headless command core, same-bitness Windows injection, production x64/Win32 target DLL bootstrap, official MinHook 1.3.4, all 13 documented Winsock signatures (the original-compatible x64 default omits WSARecvEx), bounded non-blocking capture ring, dedicated address/frame writer, live target counters/speed mode, synchronous filter matching/mutation/interception and actual injected-process TCP packet delivery over the pkt pipe; native SQLite, editors, plaintext XML export, private-station OS clipboard, original Vue/HexView/export/clipboard buttons and restart. Non-null overlapped WSA completion filtering, pre-entry suspended wake choreography, shell injection UI/x86 helper, SendPacket/GetSocketInfo, proxy/executor engines and complete acceptance remain unimplemented'
