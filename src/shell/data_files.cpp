...
Json DataService::PrepareParentExport(const std::string& method,const Json& args){
    ...
    if(kind=="sb"){
        CheckBackupParts(args);bool selected=B(args,"systemConfig")||B(args,"proxySet")||B(args,"proxyAccount")||B(args,"injectSet")||B(args,"whiteList")||B(args,"blackList");for(const auto& part:parts)selected=selected||B(args,part.c_str());
...
Json DataService::ApplyImport(const std::string& method,const Json& args,std::string_view bytes){
    ...
    if(kind=="sc"){
        ...
        if(kind=="sb"){
            ...
            for(const auto& n:root.nodes)if(n.name!="SystemConfig"&&n.name!="ProxyMode"&&n.name!="ProxyAccountList"&&n.name!="InjectMode"&&n.name!="WhiteList"&&n.name!="BlackList"&&n.name!="FilterList"&&n.name!="SendList"&&n.name!="RobotList"&&n.name!="WareHouseList")throw std::runtime_error("尚未实现备份分组："+n.name+"；已取消整次导入，未修改数据库");
